...
HERE Mobility SDK version 1.0.2 supports iOS version **9.0** or later.

### 2.2. 3rd Party Packages

The HERE Mobility SDK uses GRPC libraries.
...
```ruby
pod HereSDKDemandKit, '~>1.0.2'
pod HereSDKMapKit, '~>1.0.2'
```
...
If the `appId` and `appSecret` are missing from the file, the SDK will fail to initialize.
...
<key>HereMobilitySDKAppSecret</key>
